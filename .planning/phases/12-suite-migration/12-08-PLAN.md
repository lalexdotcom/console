...
must_haves:
  truths:
    - "tests/common/suites/scopes.suite.ts exports a named constant scopesSuite of type Suite"
    - "All 9 scope tests (SCOPE-01 through SCOPE-04) are present"
  ...
  artifacts:
    - path: "tests/common/suites/scopes.suite.ts"
      provides: "Declarative scopes suite object"
      exports: ["scopesSuite"]
      contains: "export const scopesSuite"
  key_links:
    - from: "tests/common/suites/scopes.suite.ts"
...
<objective>
Migrate `tests/common/scopes.suite.ts` (factory pattern) to a declarative `Suite`
object at `tests/common/suites/scopes.suite.ts`.

Purpose: No suite-level setup required. Browser-guard conditions remain inline.

Output: New file `tests/common/suites/scopes.suite.ts` exporting `scopesSuite`.
The old factory file `tests/common/scopes.suite.ts` is deleted in Plan 12-10.
</objective>
...
<task id="T-01">
  <title>Create tests/common/suites/scopes.suite.ts with complete Suite object</title>
  <action>
    Create `tests/common/suites/scopes.suite.ts` with the following complete content.
    Import path adjustment: `../../src` → `../../../src`.

    There is NO `setup` field — scopes tests only need `adapter.setup()`.
    ...
    Total test cases: 9 (2 SCOPE-01 + 2 SCOPE-02 + 3 SCOPE-03 + 2 SCOPE-04).
  </action>
  <read_first>
    - tests/common/scopes.suite.ts — source factory; verify ALL 9 test cases are captured
    - tests/common/suites/suite.ts — Suite interface (post 12-01)
  </read_first>
  <acceptance_criteria>
    - File exists: `ls tests/common/suites/scopes.suite.ts` exits 0
    - `grep "export const scopesSuite" tests/common/suites/scopes.suite.ts` matches
    - `grep -c "run: async" tests/common/suites/scopes.suite.ts` returns 9
    - `grep "setup:" tests/common/suites/scopes.suite.ts` returns no output
    - `grep "makeSuite" tests/common/suites/scopes.suite.ts` returns no output
...
- [ ] `ls tests/common/suites/scopes.suite.ts` exits 0
- [ ] `grep "export const scopesSuite: Suite" tests/common/suites/scopes.suite.ts` matches
- [ ] `grep -c "run: async" tests/common/suites/scopes.suite.ts` returns 9
- [ ] `grep "makeSuite" tests/common/suites/scopes.suite.ts` returns no output

## must_haves

- file_exists: `tests/common/suites/scopes.suite.ts` exists
- named_export: `export const scopesSuite: Suite` present
- no_setup: no `setup` field
- test_count: 9 TestCase objects
