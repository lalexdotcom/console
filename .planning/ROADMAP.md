...
Plans:
- [x] TBD (run /gsd-plan-phase 15 to break down) (completed 2026-04-01)

### Phase 16: suite-adapter-refactor
...
Plans:
- [x] 16-01-foundation-PLAN.md — Create LogOutput type, update TestAdapter (parse, capture→LogOutput[], drop logger), update TestCase (add check), rewrite runner (capture-then-check)
- [x] 16-02-node-adapters-PLAN.md — Add parse() + updated capture() to json, logfmt, pretty adapters
- [x] 16-03-tty-browser-adapters-PLAN.md — Add parse() + updated capture() to TTY and browser adapters
- [x] 16-04-simple-suites-PLAN.md — Migrate levels, scopes, options suites to run()/check() pattern
- [x] 16-05-medium-suites-PLAN.md — Migrate mixins, prefix, formats suites (inline parsers → LogOutput fields)
- [x] 16-06-spinners-suite-PLAN.md — Migrate spinners suite (fake timers, multi-capture merge, icon → spinnerState)
